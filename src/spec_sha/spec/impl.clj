...
(ns ^{:skip-wiki true}
  ...
  (:require
    [spec-sha.spec :as s]
    [spec-sha.spec.protocols :as protocols
...
(defmethod s/expand-spec 'spec-sha.spec/&
  [[_ re & preds]]
  {:clojure.spec/op 'spec-sha.spec/&
   :spec re
   :preds (vec preds)})

(defmethod s/create-spec 'spec-sha.spec/&
  [{:keys [spec preds]}]
  ...
  (let [{:keys [::s/op ps ks forms splice p1 rep+ maybe amp] :as p} (#'s/reg-resolve! p)]
    ...
    (when p
      (case op
        ::s/accept nil
        nil p
        ::s/amp (list* 'spec-sha.spec/& (resolve-form amp) (resolve-forms forms))
        ::s/pcat (if rep+
                 (list `s/+ rep+)
