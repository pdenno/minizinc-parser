...
(ns
  ...
  (:require [spec-sha.spec.protocols :as protocols
             :refer [conform* unform* explain* gen* #_with-gen* describe* Spec Schema]] ; POD I put Spec and Schema here
            [clojure.walk :as walk]
            [spec-sha.spec.gen :as gen])
  #_(:import [spec-sha.spec.protocols Spec Schema])) ; POD I commented this out.
...
(defn conform
  "Given a spec and a value, returns :spec-sha.spec/invalid
...
(defmacro &
  ...
  [re & preds]
  `(resolve-spec '~(explicate (ns-name *ns*) `(spec-sha.spec/& ~re ~@preds))))
...
(defmacro conformer
  ...
  (possibly converted) value or :spec-sha.spec/invalid, and returns a
...
(defmacro fdef
  ...
  instrument, tested by the runner spec-sha.spec.test/check, and (if
  ...
  (s/fdef clojure.core/symbol
    ...
    :ret symbol?)"
  [fn-sym & specs]
  `(spec-sha.spec/def ~fn-sym (spec-sha.spec/fspec ~@specs)))
...
(defop inst-in
  ...
  [start end]
  :gen #(spec-sha.spec.gen/fmap (fn [^long d] (java.util.Date. d))
         (spec-sha.spec.gen/large-integer* {:min (inst-ms start) :max (inst-ms end)}))
...
(defop int-in
  ...
  [start end]
  :gen #(spec-sha.spec.gen/large-integer* {:min start :max (dec end)})
...
(defop double-in
  ...
  :gen #(spec-sha.spec.gen/double* m)
  (and double?
    ...
    `(do
       ...
       (defmethod create-spec '~op
         ...
         (let [primary# (:primary ~'mform)
               preds# (:preds ~'mform)
               a# (:args ~'mform)
               comp-spec# (concat (list 'spec-sha.spec/and- (cons '~(explicate ns-name spec-op) a#)) preds#)]
